...
class RtcClient {
  ...
  collectStats(){
    ...
    this.timer = setInterval(async () => {
      let videoStats = await this.client_.getRemoteVideoStats()
      let audioStats = await this.client_.getRemoteAudioStats()
      let osInfo = getOS()
      let browser = getBrowser()

      console.log(osInfo, audioStats, videoStats, )

      Object.entries(audioStats).map(([key, value]) => {
        let audioData = { userid: key, ...value, ...osInfo, ...browser }
        this.mixPanel.sendEvent('HOST_AUDIO_STATS', audioData);
      })
  
      Object.entries(videoStats).map(([key, value]) => {
        let videoData = { userid: key, ...value, ...osInfo, ...browser }
        this.mixPanel.sendEvent('HOST_VIDEO_STATS', videoData);
      })
    }, 5000);
  }
}
